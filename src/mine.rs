                ...
                while let Some(msg) = message_receiver.recv().await {
                    match msg {
                        ServerMessage::StartMining(challenge, nonce_range, cutoff) => {
                                    ...
                                    std::thread::spawn({
                                        ...
                                        move || {
                                            ...
                                            loop {
                                                ...
                                                if nonce % 100 == 0
                                                    && hash_timer.elapsed().as_secs().ge(&cutoff)
                                                    && best_difficulty.ge(&8)
                                                {
                                                    break;
                                                }
                            ...
                            bin_data[1..17].copy_from_slice(&best_hash_bin);
